...
export async function getContentDetails(id: string): Promise<ContentDetails | null> {
  // Simulate API delay
  ...
  // Generate mock cast
  const mockCast: CastMember[] = Array.from({ length: 12 }).map((_, index) => ({
    id: `actor-${index + 1}`,
    name: `Actor ${index + 1}`,
    character: `Character ${index + 1}`,
    profilePath: `/placeholder.svg?height=300&width=300&text=Actor${index + 1}`,
    order: index,
  }))

  // Generate mock reviews
  const generateReviews = (count: number, type: "user" | "critic"): Review[] => {
    return Array.from({ length: count }).map((_, index) => ({
      id: `${type}-review-${index + 1}`,
      author: `${type === "critic" ? "Critic" : "User"} ${index + 1}`,
      authorImage: `/placeholder.svg?height=100&width=100&text=${type}${index + 1}`,
      title: `${type === "critic" ? "Professional" : "User"} Review ${index + 1}`,
      ...
      publication: type === "critic" ? `Publication ${index + 1}` : undefined,
    }))
  }

  // Generate mock episodes for series
  const generateEpisodes = (seasonNumber: number, episodeCount: number): Episode[] => {
    return Array.from({ length: episodeCount }).map((_, index) => ({
      id: `s${seasonNumber}e${index + 1}`,
      title: `Episode ${index + 1}`,
      episodeNumber: index + 1,
      seasonNumber,
      overview: `This is the overview for Season ${seasonNumber}, Episode ${index + 1}. It describes the main plot points and events that occur in this episode.`,
      thumbnailPath: `/placeholder.svg?height=720&width=1280&text=S${seasonNumber}E${index + 1}`,
      duration: `${Math.floor(Math.random() * 30) + 30}m`,
      airDate: `${Math.floor(Math.random() * 28) + 1}/${Math.floor(Math.random() * 12) + 1}/2023`,
  ...
  const mockSeasons = isSeries
    ? Array.from({ length: Math.floor(Math.random() * 5) + 1 }).map((_, index) => ({
        ...
        episodes: generateEpisodes(index + 1, Math.floor(Math.random() * 10) + 5),
      }))
    : undefined

  // Calculate total episodes
  const totalEpisodes = mockSeasons?.reduce((acc, season) => acc + season.episodes.length, 0) || 0

  return {
    id,
    title: `${isSeries ? "Series" : "Movie"} ${id}`,
    type: isSeries ? "series" : "movie",
    thumbnailPath: `/placeholder.svg?height=720&width=1280&text=${isSeries ? "Series" : "Movie"}${id}`,
    backdropPath: `/placeholder.svg?height=1080&width=1920&text=${isSeries ? "Series" : "Movie"}${id}`,
    logoPath: `/placeholder.svg?height=200&width=400&text=${isSeries ? "Series" : "Movie"}${id}`,
    ...
    releaseYear: 2020 + Math.floor(Math.random() * 4),
    releaseDate: `${Math.floor(Math.random() * 28) + 1}/${Math.floor(Math.random() * 12) + 1}/2023`,
    maturityRating: ["TV-MA", "TV-14", "PG-13", "R"][Math.floor(Math.random() * 4)],
    duration: isSeries ? `${mockSeasons?.[0].episodes[0].duration}` : `${Math.floor(Math.random() * 60) + 90}m`,
    genres: [
      ["Action", "Adventure", "Sci-Fi"],
      ["Comedy", "Romance"],
      ["Drama", "Thriller"],
    ...
    matchPercentage: Math.floor(Math.random() * 30) + 70,
    cast: mockCast,
    director: isSeries ? undefined : `Director ${Math.floor(Math.random() * 5) + 1}`,
    writers: isSeries
      ? undefined
      : [`Writer ${Math.floor(Math.random() * 3) + 1}`, `Writer ${Math.floor(Math.random() * 3) + 4}`],
    creator: isSeries ? `Creator ${Math.floor(Math.random() * 5) + 1}` : undefined,
    seasons: mockSeasons,
    totalEpisodes,
    audioLanguages: ["English", "Spanish", "French"],
    subtitleLanguages: ["English", "Spanish", "French", "German", "Italian"],
    videoQuality: ["HD", "4K", "HDR", "Dolby Vision"][Math.floor(Math.random() * 4)],
    studio: `Studio ${Math.floor(Math.random() * 5) + 1}`,
    awards:
      Math.random() > 0.5
        ? [`Award ${Math.floor(Math.random() * 3) + 1}`, `Award ${Math.floor(Math.random() * 3) + 4}`]
        : [],
    reviews: {
