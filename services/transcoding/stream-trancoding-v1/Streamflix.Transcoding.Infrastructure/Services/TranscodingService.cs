...
public class TranscodingService : ITranscodingService
{
    ...
    private readonly IS3StorageService _storageService; private readonly IDistributedLockService _lockService;
        ...
        List<Rendition> renditions)
    {
        try
        {
            ...
            var outputFiles = await _transcoder.TranscodeVideoAsync(
                job.Id,
                sourceFilePath,
                transcodedDir,
        ...
        Dictionary<ITranscodingProfile, string> localFiles)
    {
        try
        {
            ...
            {
                throw new InvalidOperationException($"Invalid output directory path: {outputDir}");
            }

            string manifestPath = await _transcoder.CreateHlsManifestAsync(localFiles, outputDirPath);
        ...
        Dictionary<ITranscodingProfile, string> localFiles)
    {
        try
        {
            ...
            string manifestPath = await _transcoder.CreateDashManifestAsync(localFiles, outputDirPath);
